...
function Kyc({ onClose, setUser }) {
  ...
  const validateForm = () => {
    ...
    else if (!ifscPattern.test(formData.ifsc.trim())) newErrors.ifsc = 'Invalid IFSC code format';
    if (!formData.account_number.trim()) newErrors.account_number = 'Account number is required';
    else if (!/^\d{9,18}$/.test(formData.account_number.trim())) newErrors.account_number = 'Account number must be 9-18 digits';
    if (!formData.phone.trim()) newErrors.phone = 'phone number is required';
    else if (!/^\+?[\d\s\-\(\)]{10,}$/.test(formData.phone.trim())) newErrors.phone = 'Invalid mobile number';
    if (formData.gst_number.trim()) {
      const gstPattern = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/;
      if (!gstPattern.test(formData.gst_number.trim())) newErrors.gst_number = 'Invalid GST number format';
    }
    setErrors(newErrors);
