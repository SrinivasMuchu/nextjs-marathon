...
<CadDropZoneWrapper isStyled={isStyled} type={type}>
        ...
        <div className={styles["cad-dropzone-content"]} style={isStyled ? { textAlign: "center", alignItems: "center" } : {}}>
          ...
          {type ? `Supported formats:${allowedFormats.join(", ")}` : "Supported formats: STEP (.step, .stp), IGES (.igs, .iges), STL (.stl), PLY (.ply), OFF (.off), BREP (.brp, .brep), OBJ (.obj), DXF (.dxf), DWG (.dwg)"}
          </p>
