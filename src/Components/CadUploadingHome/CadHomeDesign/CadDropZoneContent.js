...
function CadDropZoneContent({ children, isStyled, type, designVariant }) {
 
    const { allowedFormats } = useContext(contextState);

    return <CadHomeDropZone isStyled={isStyled} type={type} allowedFormats={allowedFormats} designVariant={designVariant} />;
