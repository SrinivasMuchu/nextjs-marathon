  ...
  return (
    <div className={styles['industry-design-suggestion-designs']}>
      {visibleItems.map((item, index) => (
        <a key={index} href={design_type?`/library/${item.route}`:type ?`/industry/${design.industry}/${design.part}/${item.route}`:`/industry/${item.industry_id}/${item.part_name}/${item.route}`} >
