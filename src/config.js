...
export const ASSET_PREFIX_URL = "https://d3kqb7oxkf9gw0.cloudfront.net/";
...
export const allowedFilesList = [".step", ".stp", ".stl", ".ply", ".off", ".igs", ".iges", ".brp", ".brep",".obj",".3dm"];
export const publishFilesList = [".step", ".stp", ".stl", ".ply", ".off", ".igs", ".iges", ".brp", ".brep",".obj",".dxf",".dwg",".3dm"];
...
export const CAD_OUTPUT_FILES_BUCKET = 'cad-output-files';
...
export const IMAGEURLS = {
  ...
  freeTools3:`${MARATHON_ASSET_PREFIX_URL}cad-libaray-home.webp`,
  ...
  cadFileUpload:`${MARATHON_ASSET_PREFIX_URL}upload_cad_file.svg`,
  cadFilePreview:`${MARATHON_ASSET_PREFIX_URL}cube_rotate.svg`,
...
export const CAD_LIBRARY_EVENT = 'LIBRARY'
...
/** CAD viewer tool outputs: cad-output-files/viewer/{id}/{filename} */
export function buildCadViewerOutputUrl(fileId, fileName) {
  const id = encodeURIComponent(fileId || '');
  if (!id) return '';
  const safeName = encodeURIComponent(fileName || fileId);
  return `${CAD_OUTPUT_FILES_PREFIX_URL}${CAD_VIEWER_OUTPUT_PREFIX}/${id}/${safeName}`;
}
